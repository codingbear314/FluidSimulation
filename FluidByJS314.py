...
BOX_SIZE = 3
PARTICLE_RADIUS = 0.2
DIPOLE_STRENGTH = 0.05
LONDON_STRENGTH = 0.02
FORCE_RANGE = 1
PARTICLE_MASS = 1
BOUNCE_FACTOR = 0.2
        ...
        for x in range(-12, 12, 4):
            for y in range(-12, 12, 4):
                for z in range(-12, 12, 4):
                    xRandomness = random.uniform(-0.01, 0.01)
                    yRandomness = random.uniform(-0.01, 0.01)
                    zRandomness = random.uniform(-0.01, 0.01)
                    particle = Particle(vector(x/10+xRandomness, y/10+yRandomness, z/10+zRandomness), PARTICLE_RADIUS, color.blue)
    ...
    def apply_interaction(self):
        for firstObj in range(len(self.particles)-1):
            for secondObj in range(firstObj+1, len(self.particles)):
                ...
                ldn_frc = -r * LONDON_STRENGTH / distance**6
